 ...
 *       freigeben</b> – der Schreibvorgang hat stattgefunden.</li>
...
public class SlotWiederherstellungService {
    ...
    public int verwaisteAufraeumen() {
        List<VerwaisteReservierung> verwaiste = slotReservierung.verwaisteFinden(Instant.now());
        ...
        Map<String, List<VerwaisteReservierung>> nachBetrieb = verwaiste.stream()
                .collect(Collectors.groupingBy(VerwaisteReservierung::betriebId));

        int bearbeitet = 0;
        for (Map.Entry<String, List<VerwaisteReservierung>> eintrag : nachBetrieb.entrySet()) {
            bearbeitet += betriebPruefen(eintrag.getKey(), eintrag.getValue());
        }
        return bearbeitet;
    }

    private int betriebPruefen(String betriebId, List<VerwaisteReservierung> verwaiste) {
        Instant von = verwaiste.stream().map(VerwaisteReservierung::start)
                .min(Instant::compareTo).orElseThrow().minus(SUCHPUFFER);
        Instant bis = verwaiste.stream().map(VerwaisteReservierung::ende)
                .max(Instant::compareTo).orElseThrow().plus(SUCHPUFFER);

        List<BookingAppointmentDto> termine;
        try {
            termine = terminVerwaltung.kalenderAnsichtAbrufen(betriebId, von.toString(), bis.toString());
            ...
            log.warn("Kalenderansicht für Betrieb {} nicht abrufbar, {} Reservierung(en) bleiben "
                    + "unverändert: {}", betriebId, verwaiste.size(), ex.getMessage());
            return 0;
        }

        int bearbeitet = 0;
        for (VerwaisteReservierung zeile : verwaiste) {
            Optional<BookingAppointmentDto> treffer = passendenTerminSuchen(zeile, termine);

            if (treffer.isPresent()) {
                // Der Schreibvorgang hat stattgefunden. Wiederherstellen, NICHT freigeben.
                slotReservierung.bestaetigen(List.of(zeile.id()), treffer.get().getId());
                log.info("Verwaiste Reservierung {} wiederhergestellt: Termin {} existiert in Graph",
                        zeile.id(), treffer.get().getId());
            } else {
                slotReservierung.freigeben(List.of(zeile.id()));
     ...
     * Termin verfehlen und den Slot fälschlich freigeben – der teurere Fehler.
     */
    private Optional<BookingAppointmentDto> passendenTerminSuchen(
            VerwaisteReservierung zeile, List<BookingAppointmentDto> termine) {
    ...
    private boolean ueberlappt(VerwaisteReservierung zeile, BookingAppointmentDto termin) {
        try {
            Instant terminStart = ZeitzonenUmrechnung.zuInstant(termin.getStartDateTime());
            Instant terminEnde = ZeitzonenUmrechnung.zuInstant(termin.getEndDateTime());
