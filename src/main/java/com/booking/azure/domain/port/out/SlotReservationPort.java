 ...
 *
 * <pre>
 *   1. reserve(...)            → eigene Transaktion, wird sofort festgeschrieben
 *   2. POST an Microsoft Graph     → außerhalb jeder Transaktion
 *   3a. confirm(...)           → bei Erfolg
 *   3b. release(...)             → bei Fehler (Kompensation)
 * </pre>
 *
...
public interface SlotReservationPort {
     ...
     *
     * @param request Zeitfenster in UTC, Betrieb, Dienst und Mitarbeiter
     * @return Undurchsichtige Kennungen der angelegten Reservierungen,
     *         weiterzureichen an {@link #confirm} oder {@link #release}
     ...
     */
    List<Long> reserve(SlotRequest request);

    /**
     * Markiert die Reservierungen als bestätigt und verknüpft sie mit dem in
     * Graph angelegten Termin.
     *
     * @param reservierungsIds Rückgabewert von {@link #reserve}
     * @param graphTerminId    ID des Termins aus der Graph-Antwort
     */
    void confirm(List<Long> reservierungsIds, String graphTerminId);
     ...
     * @param reservierungsIds Rückgabewert von {@link #reserve}
     */
    void release(List<Long> reservierungsIds);
     ...
     */
    List<Long> reschedule(String graphTerminId, SlotRequest neuesFenster);
